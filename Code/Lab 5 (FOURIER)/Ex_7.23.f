	...
	open(unit=21,file='wave_new.23.dat',status='unknown')
	open(unit=22,file='wave_old.23.dat',status='unknown')
	open(unit=23,file='potent_new.23.dat',status='unknown')
...
c	May need to change 4.0d0 to 4.0d0*h*h
	psi(q)=0.6316187777*cdexp((im*ko*(x(q)))-((x(q)+15.0d0)**2)/(4.0d0))
	x(q+1)=x(q)+h
