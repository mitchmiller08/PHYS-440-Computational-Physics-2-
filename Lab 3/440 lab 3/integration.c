...
double functionInput(double xValue, int choice)
{
	...
	{	//function with variable change
		// x=cos(0)
		double functionVal = pow(sin(xValue),2);
	}
	return functionVal;
}
/*********************************************************************************/
...
double** rombergTableExtrapolation(double** rombergTable, int maxRombergRow)
{
	...
	for (k=1;k<maxRombergRow;k++)
	{
		...
		{
			rombergTable[k][m+k] = (pow(4,k)*rombergTable[k-1][m+k] - rombergTable[k-1][m+k-1]) / (pow(4,k)-1);
		}
	}
	return rombergTable;
}
...
int main()
{
	...
	int choice=0;
	double upperIntegralLimit; // upper limit of integration
	double lowerIntegralLimit; // lower limit of integration
